...
require_once 'TimeZone.php';/* commentaire laurent : pour eviter un bug
...
//            $this->partsecond = isset($regs[8])?(float)$regs[8]:(float)0;
			...
			//IntÈgration des millisecondes
			$micro=Date::getMicroTime();
			$tmp=explode(".", $micro);
			
			if(isset($tmp[1])){
				$this->partsecond = $tmp[1];
			}else{
				$this->partsecond = 0;
			}
	...
	function addSubTime($ops, $year=0, $month=0, $days=0, $hours=0, $minutes=0, $seconds=0) {
		...
		$tmp =  strtotime(	$ops.
							...
							$minutes.'minutes '.$ops.
							$seconds.'seconds', 
							mktime((int)$this->hour, (int)$this->minute, (int)$this->second, (int)$this->month, (int)$this->day, (int)$this->year)
						);
	
		return $tmp;
	}
	
	function subTime($year=0, $month=0, $days=0, $hours=0, $minutes=0, $seconds=0) {
	
		$tmp = $this->addSubTime('-', $year, $month, $days, $hours, $minutes, $seconds);
		if($tmp)
			return $tmp;
	...
	function addTime($year=0, $month=0, $days=0, $hours=0, $minutes=0, $seconds=0) {
    ...
    function compare($d1, $d2, $day="")
    {
        ...
        if($day==""){
			...
			if ($d1->getMinute() > $d2->getMinute()) return 1;
			if ($d1->getSecond() < $d2->getSecond()) return -1;
			if ($d1->getSecond() > $d2->getSecond()) return 1;
		}
        return 0;
    }
	...
	/**
	 * Fonction de formatage de Date FranÁais
	 * => jj/mm/aaaa
	 * Si une date mySQL est passÈe en paramËtre,
	 * la traduit, sinon, traduit la date appelant
	 */
	function date_fr($dateMySql=""){
		if($dateMySql==""){
			$an=$this->year;
			$mois=$this->month;
			$jour=$this->day;
	...
	/**
	 * Fonction de formatage de Date Anglais
	 * => aaaa+mm+jj
	 * Si une date mySQL est passÈe en paramËtre,
	 * la traduit, sinon, traduit la date appelant
	 */
	function date_gb($dateMySql=""){
		if($dateMySql==""){
			$an=$this->year;
			$mois=$this->month;
			$jour=$this->day;
	...
	/**
	 * Fonction de formatage de Date pour export mySQL
	 * => aaaa+mm+jj
	 */
	function dateToMySql(){
		...
		$dat=$an."-".$mois."-".$jour." ".$hour.":".$minute.":".$second;
		return($dat);
	}
	 
	/**
	 * Fonction de traduction de la date en mySQL
	 * => aaaa+mm+jj
	 */
	 ...
	 * la traduit, sinon, traduit la date appelant
	 */
	function dateToTimeStamp($dateMySql=""){
		//Vérifie que la date est au format mySQL
		$tmp=explode("/", $dateMySql);
		if(count($tmp)>1){
			$dateMySql=Date::convertDateToMySql($dateMySql);
		}
		...
		if($dateMySql==""){
			...
			$an=(int)substr($dateMySql, 0, 4);
			$mois=(int)substr($dateMySql, 5, 2);
			$jour=(int)substr($dateMySql, 8, 2);
			$hr=0;
			$mn=0;
			$sec=0;
		}
		
		$tms=mktime($hr, $mn, $sec, $mois, $jour, $an);
		return($tms);
	}
	
	/**
	 * Retourne le timestamp UNIX avec les millisecondes
	 */
	
	function getMicroTime(){
		list($usec, $sec) = explode(" ",microtime());
		$timeStamp=(float)$sec;
		$tmp=explode(".", (float)$usec);
		if(isset($tmp[1])){
			$microSec=substr($tmp[1], 0, 3);
		}else{
			$microSec="000";
		}
		
		$tmp=$timeStamp.".".$microSec;
		return ($tmp);
	}
}
